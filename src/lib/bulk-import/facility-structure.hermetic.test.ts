...
describe("facility structure CSV parse", () => {
  it("parses template headers and example rows", () => {
    ...
    assert.equal(parsed.rows[0]!.space, "Room 101");
  });
...
describe("facility structure plan", () => {
  it("creates parents once for repeated names and plans deterministic order", () => {
    const csv = `floor,neighborhood,space,spaceType
...
`;
  ...
  it("rejects missing parent fields and invalid space type", () => {
    const csv = `floor,neighborhood,space,spaceType
...
`;
    ...
    assert.ok(plan.issues.some((i) => i.field === "spaceType"));
  });
...
Floor 9,Anywhere,Room 1,Resident Room
`;
    const parsed = parseFacilityStructureCsv(csv);
    assert.ok(parsed.ok);
    if (!parsed.ok) return;
  ...
  it("does not silently overwrite existing space type (conflict)", () => {
    const catalog = emptyCatalog({
      units: [
        {
          id: "f1",
...
`;
  ...
  it("rejects unknown department", () => {
    const csv = `floor,neighborhood,space,spaceType,department
