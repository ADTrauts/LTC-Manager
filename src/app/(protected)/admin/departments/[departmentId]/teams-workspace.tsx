...
function TeamFormFields({
  ...
  return (
    <div className="space-y-5">
      {defaults?.operationalTypeKeys.map((key) => (
        <input key={key} type="hidden" name="operationalTypeKeys" value={key} />
      ))}
      ...
      <section className="space-y-2">
        <h3 className="text-xs font-medium text-zinc-500">Rooms this team works</h3>
        <p className="text-xs text-zinc-500">
          Choose specific rooms, or filter by Facility Room Type to select all rooms of that
          type. Membership is assigned in Employee Builder — this is not today’s assignment.
        </p>
...
export function TeamsWorkspace({
  ...
  const selected = !adding ? teams.find((team) => team.id === selectedTeamId) ?? null : null;
    ...
    <div className="space-y-3" data-testid="department-teams-panel">
      ...
      {teams.length === 0 && !adding ? (
        <EmptyState
          ...
          action={
            canManage ? (
              <Button type="button" onClick={startAdd} data-testid="add-team-empty">
                + Add Team
              </Button>
            ) : undefined
          }
      ...
      {showSplit ? (
        <div
          className="flex flex-col overflow-hidden rounded-lg border border-zinc-200 bg-white xl:min-h-[min(56vh,40rem)] xl:flex-row"
          data-testid="department-teams-split"
        >
          <div
            className="flex w-full shrink-0 flex-col border-zinc-200 xl:w-80 xl:border-r"
            data-testid="team-list-pane"
          >
            {teams.length > 0 ? (
                  ...
                  <li key={team.id}>
                    <button
                      ...
                      className={`flex w-full items-center gap-3 px-3 py-3 text-left transition-colors hover:bg-zinc-50 ${
                        selected?.id === team.id ? "bg-amber-50/60" : ""
                      }`}
              ...
              <p className="px-3 py-4 text-sm text-zinc-500">No teams yet.</p>
            )}
          </div>

          <div className="min-w-0 flex-1 p-4" data-testid="team-detail-panel">
            {adding && canManage ? (
              <div data-testid="add-team-form">
                <h3 className="text-sm font-semibold text-zinc-900">Add Team</h3>
                ...
                >
                  ...
                  <TeamFormFields key="new" catalog={catalog} employees={employees} />
              ...
              <div className="space-y-5" data-testid="team-editor">
                {canManage ? (
                  <>
              ...
              <p className="text-sm text-zinc-500">Select a team to configure name, manager, and rooms.</p>
            )}
          </div>
        </div>
      ) : null}
