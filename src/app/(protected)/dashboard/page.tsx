...
import { EmployeeStatus, LogSubmissionStatus, MealType, UnitType } from "@prisma/client";
...
export default async function DashboardPage({ searchParams }: DashboardPageProps) {
  ...
  const [
    ...
    prisma.logSubmission.findMany({
      where: { serviceDate: { gte: start, lt: end }, unit: { facilityId } },
    ...
    prisma.scheduleEntry.findMany({
      where: { date: { gte: start, lt: end }, unit: { facilityId } },
      select: { unitId: true },
    }),
    prisma.assignmentOverride.findMany({
      where: { date: { gte: start, lt: end }, employee: { facilityId } },
  ...
  const urgentRepairs = openRepairs.filter((repair) => repair.priority === "URGENT").length;

  const mealBoards = (Object.values(MealType) as MealType[]).map((meal) => {
    const rows = unitCards
      .filter((unit) => {
      ...
      {onboardingCompleteFlag ? (
        <section className="rounded-xl border border-emerald-200 bg-emerald-50 p-4">
          ...
          <ul className="mt-3 space-y-1 text-sm text-emerald-900">
            ...
            <li>{units.length > 0 ? "Done" : "Next"}: Confirm locations and serving units in Units.</li>
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-7">
        ...
        <div className="rounded-lg border border-zinc-200 bg-white p-4 shadow-sm">
          ...
          <p className="mt-2 text-2xl font-semibold text-red-700">
            {openRepairs.length}
            <span className="ml-1 text-sm font-medium text-zinc-500">({urgentRepairs} urgent)</span>
          </p>
        </div>
      </section>
