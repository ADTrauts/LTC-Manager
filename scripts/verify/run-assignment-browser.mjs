#!/usr/bin/env node
/**
 * Phase 7A Dietary Assignment browser certification orchestrator.
 */
...
const ARTIFACT_DIR = join(ROOT, "tmp", "assignment-browser-artifacts");
const PROFILE_DIR = join(ROOT, "tmp", "assignment-browser-profile");
const FIXTURE_PATH = join(ARTIFACT_DIR, "fixtures.json");

function fail(message) {
  console.error(`test:assignment-browser: FAIL — ${message}`);
  process.exit(1);
}

function run(command, args, env, label) {
  console.log(`test:assignment-browser: ${label}`);
...
async function main() {
  ...
  console.log(`test:assignment-browser: target ${target.databaseName} @ ${target.host}`);
  console.log(`test:assignment-browser: connection ${redactDatabaseUrl(url)}`);

  if (!process.env.AUTH_SECRET) {
    process.env.AUTH_SECRET = "assignment-browser-auth-secret-not-for-production";
  }
  if (!process.env.SEED_DEMO_PASSWORD) {
    process.env.SEED_DEMO_PASSWORD = "AssignmentBrowserSeed!ChangeMe";
  }
  ...
  try {
    if (manage) {
      ...
      console.log(`test:assignment-browser: admin ${redactDatabaseUrl(adminUrl)}`);
    ...
    run(
      "node",
      ["scripts/verify/assignment-browser-fixtures.mjs"],
      {
        ...baseEnv,
        ASSIGNMENT_BROWSER_FIXTURE_PATH: FIXTURE_PATH,
      },
      "assignment fixtures",
    );

    if (process.env.ASSIGNMENT_BROWSER_SKIP_BUILD !== "1") {
      run("node", ["scripts/verify/verify-build.mjs"], baseEnv, "production build");
    } else if (!existsSync(join(ROOT, ".next"))) {
      fail("ASSIGNMENT_BROWSER_SKIP_BUILD=1 but .next is missing");
    }
    ...
    console.log(`test:assignment-browser: starting next start on ${baseUrl}`);
    serverChild = spawn(
      process.execPath,
    ...
    const pw = spawnSync(
      ...
      ["playwright", "test", "-c", "playwright.assignment.config.ts", "--grep", "@ci-gate"],
      {
        ...
        env: {
          ...baseEnv,
          NODE_ENV: "production",
          ASSIGNMENT_BROWSER_BASE_URL: baseUrl,
          ASSIGNMENT_BROWSER_FIXTURE_PATH: FIXTURE_PATH,
          ASSIGNMENT_BROWSER_PROFILE_DIR: PROFILE_DIR,
          ASSIGNMENT_BROWSER_ARTIFACT_DIR: ARTIFACT_DIR,
        },
        stdio: "inherit",
        shell: false,
      },
    ...
    console.error(`test:assignment-browser: ERROR — ${String(err?.message || err)}`);
    exitCode = 1;
  } finally {
    ...
    try {
      for (const name of readdirSync(join(ROOT, "tmp"))) {
        if (name.startsWith("assignment-browser-profile")) {
    ...
    if (created && manage) {
      try {
        ...
        console.log(`test:assignment-browser: dropped ${target.databaseName}`);
      } catch (err) {
        console.error(`test:assignment-browser: drop failed — ${String(err?.message || err)}`);
  ...
  console.log("test:assignment-browser: PASS");
}
