...
async function main() {
  ...
  try {
    ...
    if (secondaryUnit?.id) {
      await db.employeeUnitAccess.upsert({
        where: {
          employeeId_unitId: { employeeId: staffEmployee.id, unitId: secondaryUnit.id },
        },
        update: {},
        create: { employeeId: staffEmployee.id, unitId: secondaryUnit.id },
      });
    }
    ...
    // Server-visible offline sync signals for Supervisor Board OfflineSync exceptions.
    await db.offlineSyncReceipt.upsert({
      where: {
        facilityId_unitId_clientCommandId: {
          facilityId: facility.id,
          unitId: primaryUnit.id,
          clientCommandId: "jf-browser-pending-retry",
        },
      },
      update: {
    ...
    await db.offlineConflict.upsert({
      where: {
        facilityId_unitId_clientCommandId: {
          facilityId: facility.id,
          unitId: primaryUnit.id,
          clientCommandId: "jf-browser-pending-conflict",
        },
      },
      update: {
        resolution: "PENDING",
      ...
      create: {
        ...
        clientCommandId: "jf-browser-pending-conflict",
        conflictCategory: "DUPLICATE_DIFFERENT_COMMAND",
        commandPayload: { clientCommandId: "jf-browser-pending-conflict" },
        authoritativeState: {},
        reasonCode: "AUTHORITATIVE_READY_EXISTS",
        resolution: "PENDING",
      },
