#!/usr/bin/env node
/**
 * Phase 9C Dietary Asset Operations browser certification orchestrator.
 */
...
const PROFILE_DIR = join(ROOT, "tmp", "asset-operations-browser-profile");
const FIXTURE_PATH = join(ARTIFACT_DIR, "fixtures.json");
const PINS_PATH = join(ARTIFACT_DIR, "pins.env");

function fail(message) {
  console.error(`test:asset-operations-browser: FAIL — ${message}`);
  process.exit(1);
}

function run(command, args, env, label) {
  console.log(`test:asset-operations-browser: ${label}`);
...
async function main() {
  ...
  console.log(`test:asset-operations-browser: target ${target.databaseName} @ ${target.host}`);
  console.log(`test:asset-operations-browser: connection ${redactDatabaseUrl(url)}`);

  if (!process.env.AUTH_SECRET) {
    process.env.AUTH_SECRET = "asset-operations-browser-auth-secret-not-for-production";
  }
  ...
  try {
    if (manage) {
      ...
      console.log(`test:asset-operations-browser: admin ${redactDatabaseUrl(adminUrl)}`);
    ...
    run(
      ...
      "asset operations fixtures",
    );

    if (process.env.ASSET_OPERATIONS_BROWSER_SKIP_BUILD !== "1") {
      run("node", ["scripts/verify/verify-build.mjs"], baseEnv, "production build");
    } else if (!existsSync(join(ROOT, ".next"))) {
      fail("ASSET_OPERATIONS_BROWSER_SKIP_BUILD=1 but .next is missing");
    }
    ...
    console.log(`test:asset-operations-browser: starting next start on ${baseUrl}`);
    serverChild = spawn(
      process.execPath,
    ...
    const pw = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...
          ...pins,
          NODE_ENV: "production",
          ASSET_OPERATIONS_BROWSER_BASE_URL: baseUrl,
          ASSET_OPERATIONS_BROWSER_FIXTURE_PATH: FIXTURE_PATH,
          ASSET_OPERATIONS_BROWSER_PROFILE_DIR: PROFILE_DIR,
          ASSET_OPERATIONS_BROWSER_ARTIFACT_DIR: ARTIFACT_DIR,
        },
        stdio: "inherit",
        shell: false,
      },
    ...
    console.error(`test:asset-operations-browser: ERROR — ${String(err?.message || err)}`);
    exitCode = 1;
  } finally {
    ...
    try {
      for (const name of readdirSync(join(ROOT, "tmp"))) {
        if (name.startsWith("asset-operations-browser-profile")) {
    ...
    if (created && manage) {
      try {
        ...
        console.log(`test:asset-operations-browser: dropped ${target.databaseName}`);
      } catch (err) {
        console.error(`test:asset-operations-browser: drop failed — ${String(err?.message || err)}`);
  ...
  console.log("test:asset-operations-browser: PASS");
}
