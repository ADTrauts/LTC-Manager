#!/usr/bin/env node
/**
 * Phase 12A Plant browser certification orchestrator.
 */
...
const RUN_ID = process.env.PLANT_BROWSER_RUN_ID || String(process.pid);
const ARTIFACT_DIR = join(
  ROOT,
  "tmp",
  process.env.PLANT_BROWSER_ARTIFACT_DIR_NAME || `plant-browser-artifacts-${RUN_ID}`,
);
const PROFILE_DIR = join(
  ROOT,
  "tmp",
  process.env.PLANT_BROWSER_PROFILE_DIR_NAME || `plant-browser-profile-${RUN_ID}`,
);
const FIXTURE_PATH = join(ARTIFACT_DIR, "fixtures.json");
const PINS_PATH = join(ARTIFACT_DIR, "pins.env");
const DIST_DIR =
  process.env.PLANT_BROWSER_DIST_DIR || process.env.NEXT_DIST_DIR || ".next-plant-browser";

function fail(message) {
  console.error(`test:plant-browser: FAIL — ${message}`);
  process.exit(1);
}

function run(command, args, env, label) {
  console.log(`test:plant-browser: ${label}`);
...
async function main() {
  ...
  console.log(`test:plant-browser: target ${target.databaseName} @ ${target.host}`);
  console.log(`test:plant-browser: connection ${redactDatabaseUrl(url)}`);

  if (!process.env.AUTH_SECRET) {
    process.env.AUTH_SECRET = "plant-browser-auth-secret-not-for-production";
  }
  if (!process.env.SEED_DEMO_PASSWORD) {
    process.env.SEED_DEMO_PASSWORD = "PlantBrowserSeed!ChangeMe";
  }
  ...
  const baseEnv = {
    ...
    PLANT_BROWSER_DIST_DIR: DIST_DIR,
  ...
  try {
    if (manage) {
      ...
      console.log(`test:plant-browser: admin ${redactDatabaseUrl(adminUrl)}`);
    ...
    if (process.env.PLANT_BROWSER_SKIP_BUILD !== "1") {
      ...
      fail(
        `PLANT_BROWSER_SKIP_BUILD=1 but ${DIST_DIR}/BUILD_ID is missing — build with NEXT_DIST_DIR=${DIST_DIR}`,
      );
    }

    run(
      "node",
      ["scripts/verify/plant-browser-fixtures.mjs"],
      {
        ...baseEnv,
        PLANT_BROWSER_FIXTURE_PATH: FIXTURE_PATH,
        PLANT_BROWSER_ARTIFACT_DIR_NAME: ARTIFACT_DIR.split("/").pop(),
      },
      "plant fixtures",
    );
    ...
    console.log(`test:plant-browser: starting next start on ${baseUrl} (distDir=${DIST_DIR})`);
    serverChild = spawn(
      process.execPath,
    ...
    const pw = spawnSync(
      "npx",
      ["playwright", "test", "-c", "playwright.plant.config.ts", "--grep", "@ci-gate"],
      {
        cwd: ROOT,
        env: {
          ...baseEnv,
          ...pins,
          NODE_ENV: "production",
          PLANT_BROWSER_BASE_URL: baseUrl,
          PLANT_BROWSER_FIXTURE_PATH: FIXTURE_PATH,
          PLANT_BROWSER_PROFILE_DIR: PROFILE_DIR,
          PLANT_BROWSER_ARTIFACT_DIR: ARTIFACT_DIR,
        },
        stdio: "inherit",
        shell: false,
      },
    ...
    console.error(`test:plant-browser: ERROR — ${String(err?.message || err)}`);
    exitCode = 1;
  } finally {
    ...
    try {
      for (const name of readdirSync(join(ROOT, "tmp"))) {
        if (name.startsWith("plant-browser-profile")) {
    ...
    if (created && manage) {
      try {
        ...
        console.log(`test:plant-browser: dropped ${target.databaseName}`);
      } catch (err) {
        console.error(`test:plant-browser: drop failed — ${String(err?.message || err)}`);
  ...
  console.log("test:plant-browser: PASS");
}
