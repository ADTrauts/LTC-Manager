#!/usr/bin/env node
/**
 * Phase 13 Product Shell (Build/Run IA) browser certification orchestrator.
 *
 * Disposable PostgreSQL only. Enables the department-operational feature flags so the full BUILD /
 * RUN surface is exercised, while keeping OPERATION_ENGINE_ENABLED and TASK_SYNC_ENABLED false.
 */
...
const RUN_ID = process.env.PRODUCT_SHELL_BROWSER_RUN_ID || String(process.pid);
const ARTIFACT_DIR = join(
  ROOT,
  "tmp",
  process.env.PRODUCT_SHELL_BROWSER_ARTIFACT_DIR_NAME || `product-shell-browser-artifacts-${RUN_ID}`,
);
const PROFILE_DIR = join(
  ROOT,
  "tmp",
  process.env.PRODUCT_SHELL_BROWSER_PROFILE_DIR_NAME || `product-shell-browser-profile-${RUN_ID}`,
);
const FIXTURE_PATH = join(ARTIFACT_DIR, "fixtures.json");
const PINS_PATH = join(ARTIFACT_DIR, "pins.env");
const DIST_DIR =
  process.env.PRODUCT_SHELL_BROWSER_DIST_DIR || process.env.NEXT_DIST_DIR || ".next-product-shell-browser";

function fail(message) {
  console.error(`test:product-shell-browser: FAIL — ${message}`);
  process.exit(1);
}

function run(command, args, env, label) {
  console.log(`test:product-shell-browser: ${label}`);
...
async function main() {
  ...
  console.log(`test:product-shell-browser: target ${target.databaseName} @ ${target.host}`);
  console.log(`test:product-shell-browser: connection ${redactDatabaseUrl(url)}`);

  if (!process.env.AUTH_SECRET) {
    process.env.AUTH_SECRET = "product-shell-browser-auth-secret-not-for-production";
  }
  if (!process.env.SEED_DEMO_PASSWORD) {
    process.env.SEED_DEMO_PASSWORD = "ProductShellSeed!ChangeMe";
  }
  ...
  const featureEnv = {
    ...
    // Must remain false for Phase 13.
  ...
  const baseEnv = {
    ...
    PRODUCT_SHELL_BROWSER_DIST_DIR: DIST_DIR,
    ...featureEnv,
  };
  ...
  try {
    if (manage) {
      ...
      console.log(`test:product-shell-browser: admin ${redactDatabaseUrl(adminUrl)}`);
    ...
    if (process.env.PRODUCT_SHELL_BROWSER_SKIP_BUILD !== "1") {
      console.log(`test:product-shell-browser: production build → ${DIST_DIR}`);
      ...
      fail(
        `PRODUCT_SHELL_BROWSER_SKIP_BUILD=1 but ${DIST_DIR}/BUILD_ID is missing — build with NEXT_DIST_DIR=${DIST_DIR}`,
      );
    }

    run(
      "node",
      ["scripts/verify/product-shell-browser-fixtures.mjs"],
      {
        ...baseEnv,
        PRODUCT_SHELL_BROWSER_FIXTURE_PATH: FIXTURE_PATH,
        PRODUCT_SHELL_BROWSER_ARTIFACT_DIR_NAME: ARTIFACT_DIR.split("/").pop(),
      },
      "product-shell fixtures",
    );
    ...
    console.log(`test:product-shell-browser: starting next start on ${baseUrl} (distDir=${DIST_DIR})`);
    serverChild = spawn(
      process.execPath,
    ...
    const pw = spawnSync(
      ...
      ["playwright", "test", "-c", "playwright.product-shell.config.ts", "--grep", "@ci-gate"],
      {
        ...
        env: {
          ...
          ...pins,
          NODE_ENV: "production",
          PRODUCT_SHELL_BROWSER_BASE_URL: baseUrl,
          PRODUCT_SHELL_BROWSER_FIXTURE_PATH: FIXTURE_PATH,
          PRODUCT_SHELL_BROWSER_PROFILE_DIR: PROFILE_DIR,
          PRODUCT_SHELL_BROWSER_ARTIFACT_DIR: ARTIFACT_DIR,
        },
        stdio: "inherit",
        shell: false,
      },
    ...
    console.error(`test:product-shell-browser: ERROR — ${String(err?.message || err)}`);
    exitCode = 1;
  } finally {
    ...
    try {
      for (const name of readdirSync(join(ROOT, "tmp"))) {
        if (name.startsWith("product-shell-browser-profile")) {
    ...
    if (created && manage) {
      try {
        ...
        console.log(`test:product-shell-browser: dropped ${target.databaseName}`);
      } catch (err) {
        console.error(`test:product-shell-browser: drop failed — ${String(err?.message || err)}`);
  ...
  console.log("test:product-shell-browser: PASS");
}
